...
[] spawn {
	...
	while {true} do {
		...
		/*if(_server != "Evolution Gaming Teamspeak" or _channel != "TaskForceRadio") then {
			
			cutText["Wrong TS Channel.","BLACK",0,true];
			waitUntil {([] call TFAR_fnc_getTeamSpeakServerName) == "Evolution Gaming Teamspeak" and ([] call TFAR_fnc_getTeamSpeakChannelName) == "TaskForceRadio"};
			cutText["","PLAIN"];
