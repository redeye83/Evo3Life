...
switch(_type) do
{
	case 0:
	{
		...
		ctrlSetText[88886, format["Message at: %1",name life_smartphoneTarget]];
	...
	case 2:
	{
		if(({side _x == west} count playableUnits) == 0) exitWith {hint format["The police is currently not accessible. Please try again later."];};
	...
	case 4:
	{
		if(({side _x == independent} count playableUnits) == 0) exitWith {hint format["Currently, there is no medic on duty. Please try it again later."];};
