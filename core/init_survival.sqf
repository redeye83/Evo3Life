		...
		while {true} do {
			...
			_channel = _channel + 1;
				if (driver (vehicle player) == player && (vehicle player) != player) then {
					...
					if( _curfreqlr == "33.4" || _curfreqlr == "33.3" || _curfreqlr == "33.2" || _curfreqlr == "33.1" ) then {
						hint "33.1 - 33.2 - 33.3 - 33.4 are blocked frequencies.";
						[(call TFAR_fnc_activeLrRadio), _channel, "33.5"] call TFAR_fnc_SetChannelFrequency;
					};
				};
			};
			...
			_channel = _channel + 1;
				...
				if(!isNil "_curfreqsr") then {
					if( _curfreqsr == "33.4" || _curfreqsr == "33.3" || _curfreqsr == "33.2" || _curfreqsr == "33.1" ) then {
						hint "33.1 - 33.2 - 33.3 - 33.4 are blocked frequencies.";
						[(call TFAR_fnc_activeSwRadio), _channel, "33.5"] call TFAR_fnc_SetChannelFrequency;
					};
				};
			};
		};
