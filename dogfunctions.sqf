...
waitUntil {life_dog_order != 5};
		...
		{
				[[dog,"bark1"],"A3L_Fnc_NearestSound",false,false,false] call BIS_fnc_MP;
				life_dog_order = 1;
		};

		if (life_dog_order == 10) then 
		{
				[[dog,"bark2"],"A3L_Fnc_NearestSound",false,false,false] call BIS_fnc_MP;
				life_dog_order = 1;
		};

		if (life_dog_order == 11) then 
		{
				[[dog,"bark3"],"A3L_Fnc_NearestSound",false,false,false] call BIS_fnc_MP;
				life_dog_order = 1;
		};
		...
		{
				[[dog,"growlsmall"],"A3L_Fnc_NearestSound",false,false,false] call BIS_fnc_MP;
				life_dog_order = 2;
		};
		...
		_nearestunitofside = [];
			...
			if ((dog distance getpos (_nearestunitofside select 0)) < 5 && (animationState (_nearestunitofside select 0)) != "Incapacitated") then 
			{
				...
				[[dog,"growl"],"A3L_Fnc_NearestSound",false,false,false] call BIS_fnc_MP;
			} else 
			{
